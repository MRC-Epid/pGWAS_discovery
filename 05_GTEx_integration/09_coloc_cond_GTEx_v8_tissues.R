...
## script to run cond. coloc for mapping protein gene pairs using GTEx eQTL data
## Maik Pietzner 21/01/2021
rm(list=ls())
...
## get all GTEx files
ii          <- dir("/home/mdp50/rds/rds-rjh234-cc-mrc-epid/People/Ellie/GTEx_v8_download/GTEx_Analysis_v8_EUR_eQTL_all_associations_csv/") 
...
res.gtex    <- lapply(tissue, function(x){
  ...
  tmp        <- paste0("grep ", gene, " ", "/home/mdp50/rds/rds-rjh234-cc-mrc-epid/People/Ellie/GTEx_v8_download/GTEx_Analysis_v8_EUR_eQTL_all_associations_csv/",
                       x, ".v8.EUR.allpairs.chr",chr.s,".csv")
  # print(tmp)
...
ii <- as.numeric(names(ii[which(ii > 1)]))
if(length(ii) > 0){
...
res.coloc <- lapply(tissue, function(x){
  ...
  naive.coloc <- data.frame(naive.coloc, 
                            res.all[ii, c("phenotype_id", "chr", "pos.hg38", "pos", "variant_id", "MarkerName", "rsid",
...
## store results
write.table(res.coloc, paste0("tissue_coloc_cond/", soma,".", gene, ".", chr.s, ".", pos.s, ".", pos.e, ".txt"), sep="\t", row.names=F)
